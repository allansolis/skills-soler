...
async function main() {
  ...
  for (const s of settings) {
    await db.insert(crmSettings).values(s).onConflictDoNothing().run();
  }
  ...
  for (const s of stages) {
    await db.insert(pipelineStages).values(s).onConflictDoNothing().run();
  }
  ...
  console.log("\n=== Seed completo ===");
  const stageCount = (await db.select().from(pipelineStages).all()).length;
  const contactCount = (await db.select().from(contacts).all()).length;
  const dealCount = (await db.select().from(deals).all()).length;
  const convCount = (await db.select().from(conversations).all()).length;
  const actCount = (await db.select().from(activities).all()).length;
  console.log(`  ${stageCount} pipeline stages`);
  console.log(`  ${contactCount} contactos`);
  console.log(`  ${dealCount} deals`);
  console.log(`  ${convCount} conversaciones`);
